...
export const createCourseHandicap = (
  ...
  if (isNaN(courseId)) {
    // Log an error message and reject the promise
    console.error('Invalid course_id');
    return Promise.reject({ message: 'Invalid course_id' });
  }
    ...
    db.query(checkCourseSql, [courseId], (courseErr: MysqlError | null, courseData: any[]) => {
      if (courseErr) {
        ...
        const insertHandicapSql = `
          ...
          (
            course_id, hole_1, hole_2, hole_3, hole_4, hole_5, hole_6, hole_7, hole_8, hole_9,
            hole_10, hole_11, hole_12, hole_13, hole_14, hole_15, hole_16, hole_17, hole_18,
            handicap_total, is_active, created_by, created_time
          )
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;
        ...
        db.query(
          insertHandicapSql,
          [
            courseId, hole_1, hole_2, hole_3, hole_4, hole_5, hole_6, hole_7, hole_8, hole_9,
            hole_10, hole_11, hole_12, hole_13, hole_14, hole_15, hole_16, hole_17, hole_18,
            handicapTotal, is_active, user.id, new Date()
          ],
